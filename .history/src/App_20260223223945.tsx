...
function LicenseScreen({ onUnlock }: { onUnlock: () => void }) {
  ...
  const handleVerify = async () => {
    ...
    // Fake network delay to feel like a real database check
    await new Promise(r => setTimeout(r, 1500));

    // Hardcoded Master Key for testing Phase 1
    if (key === "ADITYA-PRO-2026") {
      onUnlock();
    } else {
      setError("Invalid or expired license key. Please check your purchase email.");
...
// THE MAIN ROUTER: Checks if user is locked out or allowed in
